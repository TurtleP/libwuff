...
wuff_sint32 wuff_init_stream(struct wuff_handle* handle)
{
    ...
    if (chunk.id != WUFF_FORMAT_CHUNK_ID)
    {
        ...
        wuff_status = wuff_search_chunk(
            handle, &chunk, &search_offset, WUFF_FORMAT_CHUNK_ID, WUFF_DATA_CHUNK_ID);
...
wuff_sint32 wuff_buffer_fill(struct wuff_handle* handle)
{
    ...
    wuff_status = handle->callback->read(
        handle->userdata, handle->buffer.data + bytes_in_buffer, &bytes_to_read);
    WUFF_STATUS_BAIL()
...
wuff_sint32 wuff_buffer_request(struct wuff_handle* handle, wuff_uint8** buffer, size_t* samples)
{
    ...
    /* Fill the buffer some more if the requested size is bigger than the current data in the
     * buffer. */
