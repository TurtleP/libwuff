...
wuff_sint32 wuff_open(
    struct wuff_handle** handle_pointer, struct wuff_callback* callback, void* userdata)
{
...
wuff_sint32 wuff_seek(struct wuff_handle* handle, wuff_uint64 offset)
{
    ...
    wuff_status =
        handle->callback->seek(handle->userdata, handle->stream.data.offset + seek_offset);
    WUFF_STATUS_BAIL()
